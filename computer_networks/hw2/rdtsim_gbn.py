## ****************************************************************************
## ALTERNATING BIT AND GO-BACK-N NETWORK SIMULATOR
##
## This code should be used for HW2, unidirectional data-transfer protocols
## from A to B.
##
## Network properties:
...
## ****************************************************************************
##
## ==> TODO: Implement the methods for EntityA and EntityB.
##
...
## A Msg is the data unit passed from layer 5 (teacher's code, i.e., the given 
## code) to layer 4 (student's code, i.e., your code).  It contains the data 
## (bytes) to be delivered to layer 5 via the student's transport-level protocol 
## entities.
...
##
## In case of any questions, queries, or confusions, please ask the teaching 
## staff through Piazza or office hours!
##
## ****************************************************************************
...
class EntityA:
    ...
    def output(self, message):
        ...
        cur_window_size = (self.nextSeqnum - self.base + self.seqnum_limit) % self.seqnum_limit

        if cur_window_size < self.window_size:
            checksum = self.generateChecksum(self.nextSeqnum, 0, message.data)
            packet = Pkt(seqnum=self.nextSeqnum, acknum=0, checksum=checksum, payload=message.data)
            
            #save packet by sequence number
            self.packet_dict[self.nextSeqnum] = packet

            #send packet
            to_layer3(self, packet)

            #start timer if no packets in flight:
            if cur_window_size == 0:
                start_timer(self, 250)


            if TRACE > 0:
                print(f"Packet Sent With Sequence Number {self.nextSeqnum}")

            #adjust sequence number:
            self.nextSeqnum = (self.nextSeqnum + 1) % self.seqnum_limit
        else:
            if TRACE > 0:
                print("Window Full. Queuing Message.")
            self.msg_queue.append(message)

    #Generates a checksum based on a seqnum and payload combo
    def generateChecksum(self, seqnum, acknum, payload):
        seqnum_bytes = seqnum.to_bytes()
        acknum_bytes = acknum.to_bytes()
        data = seqnum_bytes + acknum_bytes + payload
    ...
    def input(self, packet):
        ...
        checksum = self.generateChecksum(seqnum=packet.seqnum, acknum=packet.acknum, payload=packet.payload)
        
        if(checksum != packet.checksum):
            if TRACE > 0:
                print("Corrupted Packet Received. Ignoring.")
            return
        
        #check if the ack is valid and within the window space, must account for wraparound
        valid_ack = False
        if self.nextSeqnum >= self.base:
            if self.base <= packet.acknum < self.nextSeqnum:
                valid_ack = True
        else: 
            if packet.acknum >= self.base or packet.acknum < self.nextSeqnum:
                valid_ack = True

        #Check if ack received is within expected range.
        if (valid_ack):

            self.base = (packet.acknum + 1) % self.seqnum_limit
            
            #All in flight packets acknowledged, stop the timer, otherwise restart it for oldest inflight packet
            ...
            if TRACE > 0:
                print(f"ACK {packet.acknum} received, window slid forward, base is now {self.base}.")

    def send_queued_messages(self):

        window_was_empty = self.base == self.nextSeqnum

        while len(self.msg_queue) > 0 and (self.nextSeqnum - self.base + self.seqnum_limit) % self.seqnum_limit < self.window_size:

            msg = self.msg_queue.pop(0)  # Assuming messages are queued in FIFO order
            checksum = self.generateChecksum(seqnum=self.nextSeqnum, acknum=0, payload=msg.data)
            packet = Pkt(seqnum=self.nextSeqnum, acknum=0, checksum=checksum, payload=msg.data)
    ...
    def timer_interrupt(self):
        ...
        if self.base <= self.nextSeqnum:
            seq_range = range(self.base, self.nextSeqnum)
        else:
            seq_range = list(range(self.base, self.seqnum_limit)) + list(range(0, self.nextSeqnum))

        for seqnum in seq_range:
            seqnum_mod = seqnum % self.seqnum_limit
            if seqnum_mod in self.packet_dict:
                packet = self.packet_dict[seqnum_mod]
                to_layer3(self, packet)
                if TRACE > 0:
                    print(f"Timeout. Resending Packet with Sequence Number {seqnum_mod}")
        if(self.base != self.nextSeqnum):
            start_timer(self, 250)
...
class EntityB:
    ...
    def __init__(self, seqnum_limit):
        self.seqnum_limit = seqnum_limit
        self.recv_base = 0
        self.last_in_order_received=-1

    # Called from layer 3, when a packet arrives for layer 4 at EntityB.
    # The argument `packet` is a Pkt containing the newly arrived packet.
    def input(self, packet):

        checksum = self.generateChecksum(packet.seqnum, packet.acknum, packet.payload)
        ...
        if(packet.seqnum == self.recv_base):
            #send to application
            to_layer5(self, Msg(packet.payload))
            self.recv_base = (self.recv_base + 1) % self.seqnum_limit


        #create packet
        ack_num = (self.recv_base-1) % self.seqnum_limit
        checksum = self.generateChecksum(seqnum=0, acknum=ack_num, payload=bytes(20))
        ack_packet = Pkt(seqnum=0, acknum=ack_num, checksum=checksum, payload=bytes(20))
        to_layer3(self, ack_packet)

        if TRACE > 0:
            print(f"Packet Sent with ACK Number {ack_num}")
    ...
    # *** YOU NO NOT NEED TO CHANGE/IMPLEMENT THIS ***
    def timer_interrupt(self):
        pass
...
##          *** CONGRATS! YOUR HOMEWORK ENDS HERE ***
##
##
## The code below is for network simulation, and you do not need to 
## implement anything. You should not try to modify ot reference 
## anything below this. If you are interested in seeing how the simulator
## is implemented, feel free to do so! However, again, please don't 
## add/delete/modify/reference anything below!
##
##
...
class Simulator:
    def __init__(self, options, cbA=None, cbB=None):
        ...
        self.tx_file = open('msg_sent.dat', 'wb')
        self.rx_file = open('msg_received.dat', 'wb')
    ...
    def run(self):
        ...
        while self.event_list:
            ...
            if ev.ev_type == EventType.FROM_LAYER5:
                if self.n_sim < self.n_sim_max:
                    self._generate_next_arrival()
                    m = bytes([random.randint(97, 122) for _ in range(Msg.MSG_SIZE)])
                    if self.trace > 2:
                        print(f'          MAINLOOP: data given to student: {m}')
                    self.n_sim += 1
                    print(f"From Layer 5: {self.n_sim}")
                    print(f"Event_list len: {len(self.event_list)}")
                    ev.ev_entity.output(Msg(m))

                    self.tx_file.write(m)
  ...
  Call ignored.''')
    ...
    def to_layer5(self, entity, message):
        ...
        self.rx_file.write(message.data)
...
## End of program.
## Modified from the assignment by Eric Eide, University of Utah
