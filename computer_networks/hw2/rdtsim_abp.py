...
class EntityA:
    ...
    def output(self, message):
        #check if we are waiting on an ack, if not, send, otherwise put packet message in buffer (queue)
    ...
    def send_message(self, message):
        ...
        #initialize packet:
        seq_bytes = self.seqnum.to_bytes()
        data = seq_bytes + message.data
        checksum = int(hashlib.md5(data).hexdigest(), 16)
        self.most_recent_packet = Pkt(seqnum=self.seqnum, acknum=0, checksum=checksum, payload=message.data)

        #send packet and start timer:
        to_layer3(self, self.most_recent_packet)
    ...
    def input(self, packet):
        ...
        if(checksum!=packet.checksum):
            if TRACE > 0:
                print("Received Corrupted ACK Packet. Ignoring.")
            return
        ...
        if(packet.acknum == self.seqnum):
            ...
            self.wait_for_ack=False
            if(self.msg_queue):
                self.send_message(self.msg_queue.pop(0))
        else:
            if TRACE > 0:
                print("Received Unexpected ACK Packet, Ignoring.")
            start_timer(self, 250)
...
class EntityB:
    ...
    def input(self, packet):
        ...
        seq_bytes = packet.seqnum.to_bytes()
        data = seq_bytes + packet.payload
        checksum = int(hashlib.md5(data).hexdigest(), 16)
        ...
        if(packet.seqnum==self.expected_seq):
            if TRACE > 0:
                print("Expected Packet Received. Sending to Layer 5.")
            to_layer5(self, Msg(packet.payload))
            to_layer3(self, Pkt(seqnum=0, acknum=self.expected_seq, checksum=self.expected_seq, payload=bytes(20)))
            self.expected_seq = 1 - self.expected_seq
        else:
            if TRACE > 0:
                print("Unexpected Packet Received. Ignoring.")
            to_layer3(self, Pkt(seqnum=0, acknum=1-self.expected_seq, checksum=1-self.expected_seq, payload=bytes(20)))
