(* Ben Life, Sinan Seslikaya, Jalen Wang, Michael Duru, William Loving - CS 4610 - HW 3B *)
(* 
...
let less_tail (e, lst) = less_tail_helper([], e, lst);;

less_tail (5, []);;
less_tail (5, [1;10;100]);;
less_tail (-1, [-5; -1; 0]);;
less_tail (10, [-5; -1; 0]);;
...
let eval (x, coefficients) = eval_helper (x, 0.0, coefficients);;

eval(2.0, [1.0; 5.0; 3.0]);;
eval(5.0, [0.0; -2.0; 0.0; 1.0]);;
eval(0.0, [0.0; -2.0; 0.0; 1.0]);;
...
let rec remove_after_e = function
    ...
    | (e, x::xs) -> 
         if x = e 
         then [x] 
         else (let rest = remove_after_e(e, xs) in
              if rest = [] 
              then [] 
              else x::rest);;

remove_after_e(5, [1;2;3;4]);;
remove_after_e(5, [1;5;3;4]);;
remove_after_e(5, []);;
...
remove_first(5, [1;2;3;4]);;
remove_first(5, [1;5;3;4]);;
remove_first(5, [1;5;5;4]);;
remove_first(5, []);;
...
remove_all(5, [1;2;3;4]);;
remove_all(5, [1;5;3;4]);;
remove_all(5, [1;5;5;4]);;
remove_all(5, []);;
...
let rec remove_if_not = function
    ...
    | (fn, x::xs) -> 
        if fn (x) 
        then [x] @ remove_if_not(fn, xs)
        else remove_if_not(fn, xs) 
;;

let is_one a = a = 1;;

remove_if_not (is_one, [2;3;4;5]);;
remove_if_not (is_one, [1;3;4;5]);;
remove_if_not (is_one, []);;
remove_if_not (is_one, [1;1;1;1]);;
